...
$url = new moodle_url('/mod/treasurehunt/view.php', ['id' => $cm->id]);
if ($userid != $USER->id) {
...
if ($CFG->version < 2022112800) { // Moodle 4 renders the heading and the description in the header.
    echo $output->heading(
        html_writer::empty_tag('img', ['src' => treasurehunt_get_proper_icon($treasurehunt, time())]) . ' ' .
...
if (
    ...
    try {
        ...
        if (has_any_capability(['mod/treasurehunt:enterplayer', 'mod/treasurehunt:managetreasurehunt'], $context)) {
            ...
            $urlparams = ['id' => $userattemptrenderable->coursemoduleid];
            ...
            if (
                (count($userattemptrenderable->attempts) || !$userattemptrenderable->outoftime)
